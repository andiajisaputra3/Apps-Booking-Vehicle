<div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
    ...
    <!-- Modal body -->
    <form id="formRole" action="{{ $role->id ? route('role.update', $role->id) : route('role.store') }}"
        method="POST">
        @csrf
        @if ($role->id)
            @method('PUT')
        ...
        <div class="grid gap-4 mb-4 sm:grid-cols-2">
            ...
            <div>
                <label for="approval_level"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Approval Level</label>
                <input type="text" name="approval_level" id="approval_level" min="1"
                    value="{{ old('approval_level', $role->approval_level ?? '') }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                    placeholder="1">
                <small class="text-gray-500"><span class="text-red-500">*</span>Kosongkan jika role ini tidak terlibat
                    dalam proses approval.</small>
            </div>
        </div>
        <button type="submit"
