...
@section('content')
                                    ...
                                    @if ($role->permissions->isNotEmpty())
                                        <div class="flex flex-wrap gap-2">
                                            @foreach ($role->permissions as $permission)
                                                <span
                                                    class=" bg-primary-500 px-2.5 py-1.5 rounded-xl text-white text-xs break-words">{{ $permission->name }}</span>
                                            @endforeach
                                        </div>
                                    @else
    ...
    <div id="modalAction" tabindex="-1" aria-hidden="true"
        ...
        <div id="modalDialog" class="relative p-4 w-full max-w-5xl h-full md:h-auto">
            <!-- Modal content -->
...
@push('scripts')
    <script>
        // Datatables
        if (document.getElementById("roleAccess-table") && typeof simpleDatatables.DataTable !== 'undefined') {
            const dataTable = new simpleDatatables.DataTable("#roleAccess-table", {
                searchable: true,
            });
        }
    </script>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function update(id) {
                $('#formRoleAccess').on('submit', function(e) {
                    e.preventDefault();
            ...
            $('#roleAccess-table').on('click', '.action', function() {
                let data = $(this).data();
                let id = data.id;
                ...
                $.ajax({
                    method: 'GET',
                    url: `/masterdata/role-access/${id}/edit`,
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    success: function(res) {
