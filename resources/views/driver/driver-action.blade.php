<div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
    ...
    <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
            {{ $driver->id ? 'Edit Vehicle' : 'Add Vehicle' }}
        </h3>
    ...
    <!-- Modal body -->
    <form id="formDriver" action="{{ $driver->id ? route('driver.update', $driver->id) : route('driver.store') }}"
        method="POST">
        @csrf
        @if ($driver->id)
            @method('PUT')
        @endif
        <div class="grid gap-4 mb-4 {{ $driver->id ? 'sm:grid-cols-1' : 'sm:grid-cols-2' }}">
            <div>
            ...
            @if ($driver->id)
                <div>
                    ...
                    <select id="status" name="status"
                        ...
                        <option value="" disabled {{ $driver->status ? '' : 'selected' }}>Pilih Ketersediaan
                            Driver</option>
                        <option value="tersedia" {{ $driver->status == 'tersedia' ? 'selected' : '' }}>Tersedia</option>
                        <option value="tidak tersedia" {{ $driver->status == 'tidak tersedia' ? 'selected' : '' }}>Tidak
                            Tersedia</option>
                    </select>
                </div>
            @endif
